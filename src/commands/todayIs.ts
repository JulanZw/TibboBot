      ...
      'deduct',
      {
        ...
        async execute(interaction, client) {
          ...
          if (interaction.user.id !== pointGiverId) {
            return await safeReply(
              interaction,
              `Only <@${pointGiverId}> can deduct points`,
            );
          }
          ...
          if (!targetUser) {
            return await safeReply(
              interaction,
              `You did not provide a target user!`,
            );
          }

          if (!amount) {
            return await safeReply(interaction, `You did not provide points!`);
          }
          ...
          if (targetUser.id === client.user?.id) {
            ...
            logWithTime(
              `${amount} points were deducted from the bot`,
              'info',
              scope,
            );
          } else {
        ...
        customize: (builder) => {
          return builder
            .addUserOption(
              userOption('target', 'The user to deduct points from'),
            )
            .addIntegerOption(
